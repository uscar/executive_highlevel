...
//
//  Created by Rui Zeng on 11/17/16.
...
{
public:
    Trajectory(const Vector3& startPoint, const Vector3& A_, const Vector3& B_, const Vector3& C_)
    : initialPoint(startPoint), A(A_), B(B_), C(C_)
    {}
    ...
    Vector3 GetPoint(float t_)
    {
        float t = t_;
        t = t - (long)t;
        return initialPoint + Vector3(A.x * t + B.x * t * t + C.x * t * t * t,
                                    A.y * t + B.y * t * t + C.y * t * t * t,
                                    A.z * t + B.z * t * t + C.z * t * t * t);
    }
    
    Vector3 GetDerivate(float t_)
    {
        float t = t_;
        t = t - (long)t;
        return Vector3(A.x + B.x * t * 2.0f + C.x * t * t * 3.0f,
                       A.y * t + B.y * t * 2.0f + C.y * t * t * 3.0f,
                       A.z * t + B.z * t * 2.0f + C.z * t * t * 3.0f);
    }
    
    
private:
    Vector3 A, B, C, initialPoint; //Coefficient to decribe the cruve from 0 to 1;
};
